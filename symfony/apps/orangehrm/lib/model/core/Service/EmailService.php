...
class EmailService extends BaseService {
    ...
    private function _getMessage() {
        ...
        if (empty($this->messageFrom)) {
            ...
            $this->messageFrom = array($this->emailConfig->getSentAs() => 'SunTechnologies');
        }
    ...
    public function sendEmailUsingTemplate( $emailName , $info , $to) {

        if( $emailName == 'accountCreate'){
            $templateSubject = 'orangehrmAdminPlugin/modules/admin/templates/mail/en_US/accountCreate/accountCreateSubject.txt';
            $templateBody    = 'orangehrmAdminPlugin/modules/admin/templates/mail/en_US/accountCreate/accountCreateBody.txt';
            $logMessage = "Account creation email was sent to ".$to;

        }elseif( $emailName == 'rejectedTimesheet'){
            $templateSubject = 'orangehrmTimePlugin/modules/time/mail/en_US/RejectedTimesheetMail/RejectdTimsheetSubject.txt';
            $templateBody    = 'orangehrmTimePlugin/modules/time/mail/en_US/RejectedTimesheetMail/RejectdTimsheetBody.txt';
            $logMessage = "timesheet rejection email was sent to". $to;
        }

        $subject = $this->readFile($templateSubject);
        $body    = $this->readFile($templateBody);
        
        $emailBody    = $this->replaceContent($body, $info);
        $emailSubject = $this->replaceContent($subject, $info);

        $message = Swift_Message::newInstance();
        $message->setTo($to);
        $message->setFrom(array($this->emailConfig->getSentAs() => 'SunTechnologies'));
