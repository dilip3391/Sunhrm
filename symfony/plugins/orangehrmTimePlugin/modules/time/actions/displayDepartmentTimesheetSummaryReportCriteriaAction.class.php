<?php
class displayDepartmentTimesheetSummaryReportCriteriaAction extends baseTimeAction {
    ...
    */
  	public function execute($request) {
        ...
        $this->form = new displayDepartmentTimesheetSummaryReportForm();
        ...
        if ($request->isMethod('post')) {
        	...
        	$offset = ($pageNumber >= 1) ? (($pageNumber - 1) * $limit) : ($request->getParameter('pageNo', 1) - 1) * $limit;
       		...
       		if( 
                ...
                //$this->form->setSelectedEmployee($selectedEmployeeName, $employeeId);
        	 	$allData=array();
        	 	$demoObject=array();
                $selectedOption = '';
                if (!empty($employeeId)) {
                    $selectedOption = "employee_search";
                    $result = $this->getTimesheetService()->getTimeSheetOfEmployee($employeeId, $fromDate, $toDate, $selectedStatus, $limit, $offset);
                    ...
                    $result = $this->getTimesheetService()->getTimeSheetsByDepartmentAndStatusDateRange($selectedDepartment, $fromDate, $toDate, $selectedStatus, $limit, $offset);
                }
    			...
    			if(isset($download) && $download!=null  && $download=='download' ){
                    if (isset($timeSheetOfMonth) && !empty($timeSheetOfMonth)) {
                        ...
                        $url = url_for('time/displayDepartmentTimesheetSummaryReportCriteria');
