...
class viewLeaveListAction extends baseLeaveAction {
    ...
    public function execute($request) {
        ...
        if( $request->getParameter('hdnAction') == 'download') {
            $this->downloadReport($searchParams);         
            return sfView::NONE;
        }
    ...
    protected function searchLeaveRequests($searchParams, $page , $isCSVPDFExport = false) {
        $result = $this->getLeaveRequestService()->searchLeaveRequests($searchParams, $page, $isCSVPDFExport, false, true, true);
        return $result;
    }
    ...
    protected function downloadReport($searchParams) {
        $list = $this->searchLeaveRequests($searchParams, 0, true);
       
        $response = $this->getResponse();
        $response->setHttpHeader('Pragma', 'public');
        $response->setHttpHeader("Content-type", "application/csv");
        $response->setHttpHeader("Content-Disposition", "attachment; filename=LeaveData.csv");
        $response->setHttpHeader('Expires', '0');
        $content = "Date,Employee Name,Leave Type,Leave Balance(Days),Number of Days,Status,Comments\n";
           
        foreach ($list as $leaveRequest) {
            $leaveDateRange = $leaveRequest->getLeaveDateRange();
            $empName        = $leaveRequest->getEmployee()->getFirstAndLastNames();
            $leaveTypeName  = $leaveRequest->getLeaveTypeName();
            $leaveBalance   = $leaveRequest->getLeaveBalance();
            $numOfDays      = $leaveRequest->getNumberOfDays();
            $leaveStatus    = $leaveRequest->getLeaveBreakdown();
            $comments       = $leaveRequest->getComments();
            $content .= "$leaveDateRange, $empName, $leaveTypeName, $leaveBalance, $numOfDays, $leaveStatus, $comments\n";
        }
