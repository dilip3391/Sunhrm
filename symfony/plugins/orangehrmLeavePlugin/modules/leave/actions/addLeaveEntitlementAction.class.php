...
class addLeaveEntitlementAction extends sfAction {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($this->form->isValid()) {
                ...
                if (!isset($bulkFilter['bulk_assign'])) {
                    ...
                    if($leaveEntitlement->getEntitlementType() == 2 && $this->addMode == true ) {
                        
                        $employee = $this->getEmployeeService()->getEmployee($leaveEntitlement->getEmpNumber());
                        $joinedDate = $employee->getJoinedDate(); 

                        list($Y, $m, $d) = explode("-", $joinedDate);
                        if( date("Y") > $Y){
                            $leave_accural_mul = date("m") - 1 ;
                        }else{
                            $leave_accural_mul = date("m") - $m - 1 ;
                        }
                        $leaveEntitlement->setNoOfDays( $leaveEntitlement->getNoOfDays() * $leave_accural_mul );
                    }else{
                        $leaveEntitlement->setNoOfDays($leaveEntitlement->getNoOfDays());
                    }
    ...
    protected function getFiltersFromEntitlement($id) {
        ...
        $filters = array(
            ...
            'entitlement_type' => $entitlement->getEntitlementType()
        );
    ...
    protected function getLeaveEntitlement($values) {
       ...
       if(isset($values['filters']['bulk_assign'])){
           ...
           $leaveEntitlement->setNoOfDays($values['entitlement']);
            ...
            $leaveEntitlement->setEntitlementType($values['entitlement_type']);
