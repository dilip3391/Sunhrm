...
class EmployeeDao extends BaseDao {
   ...
   /**
	...
	/**
      * Fetching employee's Supervisor list/Reporting managers(RK)
    **/
    public function getSupervisorListForEmployee($empNumber){
        ...
        try{
            ...
            if($empNumber!=null){
                ...
                $q="SELECT  emp_firstname,emp_middle_name,emp_lastname 
                FROM hs_hr_employee,hs_hr_emp_reportto,ohrm_emp_reporting_method
                WHERE erep_sup_emp_number = emp_number AND erep_reporting_mode=reporting_method_id AND erep_sub_emp_number =$empNumber"; 
    ...
    * Purpose : To load all the employees under the selected department(work_station)
    */
    public function getDepartmentEmployees($workStation)
    {

        /*$q = Doctrine_Query::create()
                ->select('emp.empNumber')
                ->from('Employee emp')
                ->where('emp.work_station = ?', $workStation);
                echo $q->getSqlQuery();exit;
        var_dump($q->getParameters());exit;
        $result = $q->execute();
        var_dump($result);exit;*/
        $query = "select emp_number as empNumber, emp_firstname as firstName, emp_middle_name as middleName, emp_lastname as lastName, termination_id from hs_hr_employee where role_in_department IS NULL AND work_station = ".$workStation;
