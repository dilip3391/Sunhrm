...
class EmployeePersonalDetailsForm extends BaseForm {
    ...
    private function getPersonalInfoWidgets() {
        $widgets = array(
            ...
            'opt_out_of_birthday_mails' => new sfWidgetFormInputCheckbox(
                array(
                    'value_attribute_value' => '1', 
                    'default' => $this->employee->opt_out_of_birthday_mails
                )
            ),

            'check_for_confirmation' => new sfWidgetFormInputCheckbox(
                array(
                    'value_attribute_value' => '1', 
                    'default' => $this->employee->check_for_confirmation
                )
            )
        );
        ...
        $checkStatus = ($this->employee->opt_out_of_birthday_mails != null) ? 'checked' : '';
        $checkStatusConfirmation = ($this->employee->check_for_confirmation != null) ? 'checked' : '';
        
        $widgets['opt_out_of_birthday_mails']->setAttribute(
            'checked', 
            $checkStatus
        );

        $widgets['check_for_confirmation']->setAttribute(
            'checked', 
            $checkStatusConfirmation
        );
    ...
    private function getPersonalInfoValidators() {
        ...
        $validators = array(
            ...
            'opt_out_of_birthday_mails' => new sfValidatorInteger(array('required' => false)),
            'check_for_confirmation' => new sfValidatorInteger(array('required' => false)),
    ...
    public function getEmployee() {
        ...
        if ($this->personalInformationPermission->canUpdate()) {
            ...
            $employee->opt_out_of_birthday_mails = $this->getValue('opt_out_of_birthday_mails');
            $employee->check_for_confirmation = $this->getValue('check_for_confirmation');
