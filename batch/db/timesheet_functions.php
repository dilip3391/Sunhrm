...
/*SELECT ohrm_attendance_record.employee_id, SEC_TO_TIME(SUM(TIME_TO_SEC(actual_working_hours))) as total_dur, emp_firstname, emp_lastname, hs_hr_emp_reportto.erep_sup_emp_number FROM `ohrm_attendance_record` 
	JOIN hs_hr_employee on hs_hr_employee.emp_number = ohrm_attendance_record.employee_id 
	JOIN hs_hr_emp_reportto on hs_hr_emp_reportto.erep_sub_emp_number = ohrm_attendance_record.employee_id 
	WHERE punch_in_user_time BETWEEN '2017-11-26' AND '2017-12-02' 
	AND ohrm_attendance_record.employee_id in (SELECT DISTINCT erep_sub_emp_number as subordinates FROM hs_hr_emp_reportto) 
	AND hs_hr_employee.termination_id IS NULL GROUP BY ohrm_attendance_record.employee_id 
*/
	
/*
SELECT ohrm_attendance_record.employee_id, SEC_TO_TIME(SUM(TIME_TO_SEC(actual_working_hours))) as total_dur, emp_firstname, emp_lastname, hs_hr_emp_reportto.erep_sup_emp_number
FROM `ohrm_attendance_record` 
JOIN hs_hr_employee on hs_hr_employee.emp_number = ohrm_attendance_record.employee_id 
JOIN hs_hr_emp_reportto on hs_hr_emp_reportto.erep_sub_emp_number = ohrm_attendance_record.employee_id  
WHERE punch_in_user_time BETWEEN '2017-11-26' AND '2017-12-02' 
AND ohrm_attendance_record.employee_id in (SELECT DISTINCT erep_sub_emp_number as subordinates FROM hs_hr_emp_reportto) 
AND hs_hr_employee.termination_id IS NULL GROUP BY ohrm_attendance_record.employee_id,hs_hr_emp_reportto.erep_sup_emp_number*/